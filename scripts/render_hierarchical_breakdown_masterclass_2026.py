...
- Motor: Renderizado Paralelo Multiprocessing (8x Turbo Cores)
==============================================================================
"""
...
def synthesize_hierarchical_audios():
    ...
    for idx, item in enumerate(HIERARCHICAL_MODULES):
        ...
        if not master_aac.exists() or master_aac.stat().st_size < 1000:
            async def generate():
...
# Variables de multiprocessing a nivel de módulo
_WORKER_TIMELINE = []
_WORKER_TOTAL_DUR = 0.0
_WORKER_FRAMES_DIR = None
_WORKER_AVATAR_PNG = None

def init_worker(timeline, total_dur, frames_dir_str, avatar_path_str):
    global _WORKER_TIMELINE, _WORKER_TOTAL_DUR, _WORKER_FRAMES_DIR, _WORKER_AVATAR_PNG
    _WORKER_TIMELINE = timeline
    _WORKER_TOTAL_DUR = total_dur
    _WORKER_FRAMES_DIR = Path(frames_dir_str)
    
    # Pre-cargar y redimensionar el avatar UNA SOLA VEZ en memoria RAM
    raw_av = Image.open(avatar_path_str).convert("RGBA")
    av_h = 880
    av_w = int(raw_av.width * (av_h / raw_av.height))
    _WORKER_AVATAR_PNG = raw_av.resize((av_w, av_h), Image.Resampling.BILINEAR)

def render_frame_worker(args):
    f_idx, t, active_mod_idx = args
    active_mod = _WORKER_TIMELINE[active_mod_idx]
    ...
    # Cargar fuentes localmente por worker
    try:
        ...
        font_badge = font_title = font_concept = font_box_title = font_header = font_indent = font_bullet = font_sub = font_top = ImageFont.load_default()

    avatar_png = _WORKER_AVATAR_PNG
    ...
    # Breakdown Jerárquico con Sangría
    content_x = 620
    content_y = 95
    content_w = 1240
    ...
    progress_pct = t / _WORKER_TOTAL_DUR
    draw.rectangle([0, HEIGHT - 6, int(WIDTH * progress_pct), HEIGHT], fill=(212, 175, 55))

    frame_file = _WORKER_FRAMES_DIR / f"h_frame_{f_idx:06d}.jpg"
    frame.save(frame_file, quality=85)
...
def render_hierarchical_masterclass():
    ...
    # 1. Sintetizar audios
    synthesize_hierarchical_audios()
    ...
    avatar_src = str(ROOT / "assets" / "avatar_transparent.png")
    ...
    print(f"\n[FASE 3/4] Renderizando {total_frames} fotogramas Full HD en Paralelo (8x Turbo Cores)...")

    render_tasks = []
    for f_idx in range(total_frames):
        t = f_idx / FPS
        active_mod_idx = len(timeline) - 1
        for m_i, entry in enumerate(timeline):
            if entry["start"] <= t <= entry["end"]:
                active_mod_idx = m_i
                break
        render_tasks.append((f_idx, t, active_mod_idx))

    cores = min(8, cpu_count() or 4)
    with Pool(processes=cores, initializer=init_worker, initargs=(timeline, total_duration, str(frames_dir), avatar_src)) as pool:
        for idx, _ in enumerate(pool.imap_unordered(render_frame_worker, render_tasks, chunksize=50)):
            if idx % 600 == 0:
                print(f"    -> [TURBO 8X] Fotograma {idx}/{total_frames} ({idx/total_frames*100:.1f}%)...")
    ...
    # Abrir inmediatamente el video en Windows Media Player y Explorer
    subprocess.run(["cmd", "/c", "start", "", str(final_output)])
