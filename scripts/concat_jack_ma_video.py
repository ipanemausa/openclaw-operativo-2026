...
list_file = lipsync_dir / "concat_jack_ma.txt"
with open(list_file, "w", encoding="utf-8") as f:
    for v in vids:
        clean_path = str(v).replace("\\", "/")
        f.write(f"file '{clean_path}'\n")

out_video = Path(r"C:\openclaw\hb-jewelry\public\videos\jack_ma_style\jack_ma_style_b2b_master.mp4")
...
if res.returncode == 0:
    ...
    print(f"✅ VIDEO JACK MA STYLE COMPLETADO: {out_video} ({size_mb:.2f} MB)")
else:
